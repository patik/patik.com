...
We've been using `options` as an object that contains all the _other_ stuff, besides the query key & query function. But the whole point of our query hooks is to make sure the key and function are consistent. Therefore, let's continue using `options` _only_ for the other stuff.

This means the type usually needs to be changed:

```tsx
// Before
function useMyHook(requestArgs: RequestArgs, options: UseQueryOptions) {
    return useQuery(/* ... */)
}

// After
function useMyHook(requestArgs: RequestArgs, options: Omit<UseQueryOptions, 'queryKey' | 'queryFn'>) {
    return useQuery(/* ... */)
}
```

### Be mindful of options we _do_ want to override

Those should still go _above_ `...options`:
...
_Recall that we have our own `useInfiniteQuery` that wraps around the third-party library's hook._
...
![TypeScript error showing pageParam missing from QueryFunctionContext because its type was not provided](/portfolio/pull-requests/react-query-v5-upgrade/01.webp)

Hovering over `QueryFunctionContext`, we see it takes another generic, `TPageParam`:

![QueryFunctionContext type signature highlighting the TPageParam generic](/portfolio/pull-requests/react-query-v5-upgrade/02.webp)

The solution is to assign `number` to that param:

![The useBookmarks query with number supplied as QueryFunctionContext's page parameter type](/portfolio/pull-requests/react-query-v5-upgrade/03.webp)
...
![Annotated usePhoto request showing why its ID must be required even though the hook accepts an optional ID](/portfolio/pull-requests/react-query-v5-upgrade/04.webp)
...
![The usePhoto query with an optional request ID guarded by the enabled option](/portfolio/pull-requests/react-query-v5-upgrade/05.webp)
...
![The usePhoto query combining the request ID and caller's enabled option in one Boolean condition](/portfolio/pull-requests/react-query-v5-upgrade/06.webp)
...
![The usePhoto query using skipToken when its optional ID is undefined](/portfolio/pull-requests/react-query-v5-upgrade/07.webp)
...
![Console timeline showing isPending, isLoading, and data availability before, during, and after a query](/portfolio/pull-requests/react-query-v5-upgrade/08.webp)
...
We now log _all_ errors, whereas before we sometimes completely ignored them. We might find some issues in our logs that were happening before, but were always silenced.
...
- A bunch of `queryClient.xxxx` methods now require the object syntax
    - ![React Query v5 method signatures changing from positional arguments to object syntax](/portfolio/pull-requests/react-query-v5-upgrade/09.webp)
