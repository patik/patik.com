  ...
  var withinViewport = function(elem, settings)
  {
    ...
    try {
      ...
      var arr, i, side, // Temporary variables for looping
          ...
          config = {
            debug: settings.debug || metadata.debug || withinViewport.defaults.debug || false,
          ...
          isWithin = {
            ...
            top: function() {
              if (config.debug) { console.log('inTop', config.top, elem, 'will return: ', (elemOffset[1] >= scrollOffset[1] + config.top)); }
              return elemOffset[1] 
            ...
            right: function() {
              if (config.debug) { console.log('inRight', config.right, elem, 'will return: ', (elemOffset[0] + elem.offsetWidth <= window.innerWidth + scrollOffset[0] - config.right)); }
            ...
            bottom: function() {
              if (config.debug) { console.log('inBottom', config.bottom, elem, 'will return: ', (elemOffset[1] + elem.offsetHeight <= scrollOffset[1] + window.innerHeight - config.bottom)); }
            ...
            left: function() {
              if (config.debug) { console.log('inLeft', config.left, elem, 'will return: ', (elemOffset[0] >= scrollOffset[0] + config.left)); }
              return elemOffset[0] 
            ...
            all: function() {
              if (config.debug) {
                console.log('inAll', elem, 'scroll:' + scrollOffset + ', elem off: ' + elemOffset + '; will return ', (isWithin.top() && isWithin.right() && isWithin.bottom() && isWithin.left()));
              }
      ...
      while (i--) {
        ...
        if (/top|right|bottom|left|all/.test(side)) {
          if (isWithin[side]()) {
            ...
            if (config.debug) { console.log('Quit as soon as we find the first failure: ' + side); }
            return false;
          }
        }
      }
